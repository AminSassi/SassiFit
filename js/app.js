...
function add(id,n){
  ...
  if(r>=g&&!S.done[id]){S.done[id]=true;save();toast((e=>e?e.name:id)(all().find(x=>x.id===id))+' complete');if(navigator.vibrate)navigator.vibrate(25)}
...
function render(){
  ...
  if(done){document.getElementById('heroProgress').style.display='none';document.getElementById('heroDone').style.display='block'}
  else{document.getElementById('heroProgress').style.display='';document.getElementById('heroDone').style.display='none';document.getElementById('ring').style.strokeDashoffset=327-327*pct/100;document.getElementById('ringPct').textContent=pct+'%';document.getElementById('heroSub').textContent=rem+' reps remaining'}
  ...
  e.forEach(x=>{const r=S.reps[x.id]||0,p=Math.min(100,Math.round(r/S.goal*100)),d=r>=S.goal;const el=document.createElement('div');el.className='card'+(d?' card-done':'');el.innerHTML=`<div class="card-top"><span class="card-name">${x.name}</span><span class="card-reps"><b>${r}</b> / ${S.goal}</span></div><div class="card-bar"><div class="card-fill" style="width:${p}%"></div></div><div class="card-btns"><button class="btn btn-m" onclick="add('${x.id}',-1)">−</button><button class="btn" onclick="add('${x.id}',1)">+1</button><button class="btn btn-p" onclick="add('${x.id}',10)">+10</button><button class="btn" onclick="add('${x.id}',50)">+50</button></div>`;c.appendChild(el)});
  ...
  document.getElementById('summaryTotal').innerHTML=`<span>Total</span><span>${tot} reps</span>`;
}

function openHistory(){
  const c=document.getElementById('historyContent');
  c.innerHTML=`<div class="rec-grid"><div class="rec"><div class="rec-v">${S.best||0}</div><div class="rec-l">Best Streak</div></div><div class="rec"><div class="rec-v">${streak(S.hist||[])}</div><div class="rec-l">Current</div></div><div class="rec"><div class="rec-v">${S.most||0}</div><div class="rec-l">Most Reps</div></div><div class="rec"><div class="rec-v">${S.days||0}</div><div class="rec-l">Days Done</div></div></div><p class="label">Heatmap</p>`;
...
function renderCustom(){const l=document.getElementById('customList');l.innerHTML='';(S.custom||[]).forEach(e=>{l.innerHTML+=`<div class="cust-row"><span style="font-weight:700;font-size:14px">${e.name}</span><button class="cust-rm" onclick="rmCustom('${e.id}')">Remove</button></div>`})}
...
function updateNotif(){const b=document.getElementById('notifBtn'),info=document.getElementById('notifInfo');if(!('Notification' in window)){b.textContent='Not Supported';return}if(Notification.permission==='granted'){b.textContent='✓ Enabled';b.style.color='#30d158';if(info)info.textContent='8AM, 12PM, 6PM, 9PM when open.'}else if(Notification.permission==='denied'){b.textContent='Blocked';b.style.color='#ff4444';if(info)info.textContent='Enable in browser settings.'}else{b.textContent='Enable Notifications';if(info)info.textContent=/iPad|iPhone|iPod/.test(navigator.userAgent)&&!window.navigator.standalone?'Install to Home Screen for reminders.':''}}
